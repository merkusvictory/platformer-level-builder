...
export default function Play() {
  ...
  useEffect(() => {
    ...
    function renderFrame() {
      ...
      const c_bg     = isLight ? '#dedad3' : (isHardModeRef.current ? '#0a0510' : '#1e1b2e')
      ...
      // Grid faint lines
      ctx.strokeStyle = editModeRef.current ? 'rgba(255,255,255,0.10)' : c_grid
      ctx.lineWidth = 1
  ...
  return (
    <div
      className={`flex h-dvh overflow-hidden transition-colors duration-700 ${isHardMode ? 'hard-theme bg-[#0a0004]' : 'easy-theme'}`}
      style={isHardMode ? {} : { background: 'var(--bg-base)' }}
    >
      ...
      <div
        ...
        style={editMode || isDevMode ? {} : { background: 'var(--bg-primary)', borderColor: 'var(--border-ui)' }}
      >
        {editMode ? (
          /* ── Editor Tool Palette ── */
          <>
              ...
              {gameWon && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  ...
                  >
                    ...
                    <h2 className={`text-3xl font-black mb-2 ${isHardMode ? 'text-red-400' : ''}`} style={isHardMode ? {} : { color: 'var(--text-base)' }}>
                    ...
                    {isHardMode && hardModeData && (
                      <div className="inline-flex items-center gap-1.5 bg-red-500/20 border border-red-500/40 rounded-full px-3 py-1 mb-3">
                        ...
                        <span className="text-xs font-black" style={{ color: 'var(--text-base)' }}>{hardModeData.difficulty_estimate}/10</span>
                      </div>
                    )}
